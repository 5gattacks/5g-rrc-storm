...
export default function OAINetworkManager() {
  const [activePage, setActivePage] = useState('overview'); // 'overview' | 'config' | 'sim'
  ...
  const [sib8Config, setSib8Config] = useState({
    ...
    serialNumber: '3FF1',
    dataCodingScheme: '48',
    text: 'This is a default warning message',
    mode: '0'
  });
  ...
  useEffect(() => {
    ...
    else if (activePage === 'config') fetchConfigs();
  }, [activePage]);
  ...
  useEffect(() => {
    ...
    fetchOverview();
    const t = setInterval(fetchOverview, 2000);
  ...
  const fetchOverview = async () => {
    try {
      ...
      setOverview(null);
      setMessage({ type: 'error', text: 'Failed to load overview (backend /api/overview not ready yet)' });
    } 
  };
  ...
  const formatTaBucket = (bucket) => {
    if (!bucket) return '-';
  
    const bucketId = bucket.bucketId ?? '-';
    const taStart = bucket.taStart;
    const taEnd = bucket.taEnd;
  
    if (taStart == null || taEnd == null) {
      return `Bucket ${bucketId}`;
    }
  
    if (taStart === taEnd) {
      return `Bucket ${bucketId}: {${taStart}}`;
    }
  
    const values = [];
    for (let ta = taStart; ta <= taEnd; ta++) {
      values.push(ta);
    }
  
    return `Bucket ${bucketId}: {${values.join(',')}}`;
  };

  const renderStormStatus = (storm) => {
    if (!storm) return <p className="subtitle">No storm data available yet.</p>;

    if (!storm.underAttack) {
        return (
          <div className="card">
            <h2>RRC Storm Monitor</h2>
            ...
            <div className="kv">
              <div className="kv-row"><span>Msg4</span><span>-</span></div>
              <div className="kv-row"><span>Msg5</span><span>-</span></div>
              <div className="kv-row"><span>R</span><span>-</span></div>
              <div className="kv-row"><span>Half-open connections</span><span>-</span></div>
              <div className="kv-row"><span>Suspicious TA Bucket</span><span>-</span></div>
              <div className="kv-row"><span>Half-open from Suspicious Bucket</span><span>-</span></div>
            </div>
          </div>
        );
    }
    ...
    return (
      <div className="stack">
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '16px' }}>
              <span className="status-badge danger">ATTACK DETECTED</span>
          </div>

          <div className="kv">
            <div className="kv-row"><span>Msg4</span><span>{storm.msg4 ?? '-'}</span></div>
            <div className="kv-row"><span>Msg5</span><span>{storm.msg5 ?? '-'}</span></div>
            <div className="kv-row"><span>R</span><span>{storm.r2 != null ? `${storm.r2}%` : '-'}</span></div>
            <div className="kv-row"><span>Half-open connections</span><span>{storm.suspiciousBucket?.total ?? '-'}</span></div>
            <div className="kv-row">
              <span>Suspicious TA Bucket</span>
              <span>{formatTaBucket(storm.suspiciousBucket)}</span>
            </div>

            <div className="kv-row">
              <span>Half-open from Suspicious Bucket</span>
              <span>{storm.suspiciousBucket?.halfOpen ?? '-'}</span>
            </div>
  ...
  const renderOverviewPage = () => (
    <div className="page">
      ...
      {!overview ? (
      <p className="subtitle">No overview data yet.</p>
      ) : (
        <div className="stack">
          ...
          <div className="kv">
            <div className="kv-row">
              <span>
                {overview.core?.on ? 'ON' : 'OFF'}
              </span>
            </div>
          </div>
          <h3>gNBs</h3>
          <div className="kv">
            <div className="kv-row"><span>Total</span><span>{overview.gnbs?.total}</span></div>
            <div className="kv-row"><span>Connected</span><span>{overview.gnbs?.connected}</span></div>
            <div className="kv-row"><span>Disconnected</span><span>{overview.gnbs?.disconnected}</span></div>
          </div>
          </div>
          ...
          <div className="kv">
            <div className="kv-row">
              <span>Connected to AMF</span>
              <span>
                {!overview.gnbs.found
                  ? '-'
                  : (overview.gnbs?.gnb?.connectedToAmf ? 'YES' : 'NO')}
              </span>

            </div>
            {overview.gnbs.found && (
              <div className="kv">
                <div className="kv-row"><span>TAC</span><span>{overview.gnbs?.gnb?.config?.tac ?? '-'}</span></div>
                <div className="kv-row"><span>Band</span><span>{overview.gnbs?.gnb?.config?.band ?? '-'}</span></div>
                <div className="kv-row"><span>SSB Freq (Hz)</span><span>{overview.gnbs?.gnb?.config?.ssbFreq ?? '-'}</span></div>
                <div className="kv-row"><span>PRB</span><span>{overview.gnbs?.gnb?.config?.prb ?? '-'}</span></div>
                <div className="kv-row"><span>Numerology</span><span>{overview.gnbs?.gnb?.config?.mu ?? '-'}</span></div>
              </div>
            )}
          </div>
          </div>
          ...
          <div className="kv" style={{ marginBottom: 12 }}>
            <div className="kv-row"><span>Total</span><span>{overview.phones?.total ?? '-'}</span></div>
            <div className="kv-row"><span>Registered</span><span>{overview.phones?.registered ?? '-'}</span></div>
          </div>

        {!overview.phones.found ? (
              <p className="subtitle">UE table not available yet (waiting for AMF table)...</p>
              ) 
              : overview.phones.table.length === 0 ? (
              <p className="subtitle">No UEs in AMF table.</p>
              ) 
              : (
              <table className="simple-table">
                ...
                <tbody>
                  {overview.phones.table.map((ue, i) => (
                    <tr key={`${ue.imsi}-${i}`}>
                      <td>{ue.imsi}</td>
                      <td>{ue.mmState}</td>
                      <td>{ue.cellId}</td>
          ...
          {renderStormStatus(overview.storm)}         
        </div>
                  )}
    </div>
